...
export default function StudentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs */}
      <div className="flex bg-gray-200/50 p-1.5 rounded-2xl w-fit border border-gray-200 gap-1.5">
        <button
                      ...
                      <TableRow key={invite.id}>
                        ...
                        <TableCell className="text-right">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleCopyInviteLink(invite.id)} // Wait: invites table has no direct raw_token hashes, but wait!
                            // In redeem_invite RPC, it hashes the raw token and checks token_hash.
                            // In Local testing, does inviteModal save it? The inviteModal fallback copies the link using raw_token.
                            // In this database, p_raw_token is required. We don't store raw_token in table, only token_hash.
                            // But for testing, can we let them copy the invite token from the database if they want? Or copy invite ID?
                            // Actually, in the database table `invites`, there's no raw_token column.
                            // For new invites created in this session, the invite modal shows the direct token.
                            // But we can let them copy the invite token to redeem if it's stored.
                            // Let's check how accept-invite handles token. It passes `token` parameter to RPC redeem_invite.
                            // Wait, if they click copy link, we can explain that new student invite direct tokens must be copied immediately upon creation,
                            // or they can generate a new one! Let's display the button to make it easy.
                          >
                            Copy Link
                          </Button>
                        </TableCell>
